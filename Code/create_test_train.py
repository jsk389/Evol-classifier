...
def import_data(sfile='../Data/new_reggae_data.csv'):
    # Read in csv file
    df = pd.read_csv(sfile, header=0)
    return df
...
def preproc(df, flag):
    ...
    if flag == 'Kepler':
        ...
        #drop_cols = ['KIC', 'anu', 'anu_err', 'hsig1_err', 'b1_err','c1_err','hsig2_err', 'b2_err', 'c2_err',
        #             'numax_err','denv_err','Henv_err', 'alpha_err','beta_err', 'c3_err', 'white', 'white_err']
        drop_cols = ['KIC', 'anu', 'anu_err', 'DPi1', 'e_DPi1', 'q', 'Status', 'KALLINGER_EVSTATES', 'age', 'age_68L', 'age_68U', 'age_95L', 'age_95U', 'mass', 'mass_68L',
                     'mass_68U', 'mass_95L', 'mass_95U', 'logg', 'logg_68L', 'logg_68U', 'logg_95L', 'logg_95U', 'rad', 'rad_68L', 'rad_68U', 'rad_95L', 'rad_95U', 'logrho',
                     'logrho_68L', 'logrho_68U', 'logrho_95L', 'logrho_95U', 'mbol', 'mbol_68L', 'mbol_68U', 'mbol_95L', 'mbol_95U', 'Kepler', 'Kepler_68L',
                     'Kepler_68U', 'Kepler_95L', 'Kepler_95U', 'g', 'g_68L', 'g_68U', 'g_95L', 'g_95U', 'r', 'r_68L', 'r_68U', 'r_95L', 'r_95U',
...
if __name__=="__main__":
    ...
    # Import data
    df = import_data()#'../Data/new_Kep-as-K2_reggae_data.csv')
    ...
    # Preprocessing
    print(len(df))
    df = preproc(df, flag='Kepler')#, flag='K2')
    print(len(df))
    df = df[df['evol_overall'] != 3]
    plt.scatter(df['numax'], df['denv'], c=df['evol_overall'], s=50, alpha=0.8)
    ...
    plt.scatter(train['numax'], train['Henv'], c=train['evol_overall'])
    plt.show()
    # Create test set with unknown labels
    test = df[df['evol_overall'] == 3]

    test.to_csv('../Data/test_Kep_as_K2_'+str(label_type)+'.csv', index=False)
    train.to_csv('../Data/train_Kep_as_K2_'+str(label_type)+'.csv', index=False)
